...
const AccountIncomeList = () => {
    ...
    const { data } = useQuery(['branchIncome', currentPageNumber], async () =>
        zodSafeQuery(
            `/api/v1/transaction/income/?limit=${itemsLoadPerPage}&offset=${currentPageNumber * itemsLoadPerPage}`
        )()
    );
    console.log({ data });
    if (!data) {
...
export default AccountIncomeList;
