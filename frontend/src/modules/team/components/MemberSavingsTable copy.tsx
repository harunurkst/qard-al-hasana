...
import BlankSheet from './pdf/BlankSheet';
...
const registerFont = () => {
    Font.register({
      family: 'Nikosh',
      src: '/fonts/Nikosh.ttf', // Provide the correct path to your font file
    });
  };
...
const MemberSavingsTable: React.FC<IMemberSavingsTable> = ({ teamId, teamName, branchName, orgName, teamAddress }) => {
    ...
    const [blankSheet, setBlankSheet] = useState(null);
    const [isFontRegistered, setIsFontRegistered] = useState(false);

    // Use useEffect to register the font when the component mounts
    useEffect(() => {
      registerFont();
      setIsFontRegistered(true); // Set the state to indicate font registration success
    }, []);
        ...
        <View style={styles.mypdf}>
            ...
            <View style={styles.tableContainer}>
                ...
                {Array(25)
                    .fill()
                    .map((_, index) => {
                        const tempMatchingData = data1?.result.find((item) => item.sl === index + 1);
                        const matchingData = {...tempMatchingData}
                        ...
                        return (
                            <View style={styles.tableRow} key={index}>
                                ...
                                <View style={styles.tableCellBorder3}>
                                    <Text style={[styles.name,, { fontFamily: 'Nikosh' } ]}>{matchingData?.member_name}</Text>
                                </View>
    ...
    },[isFontRegistered,data1])
    // console.log('data2: ', data2);
    useEffect(()=>{
        console.log('isFontRegistered,data1,blankpdf',isFontRegistered,data1,blankpdf)
            },[isFontRegistered,data1,blankpdf])
